...
fn corrupt_stack() {
    let (stack_base, stack_size) = my_stack();
    let ptr = stack_base as *mut usize;
    log::info!("Writing to {:#x?} - old value {:#x?}", stack_base, unsafe { ptr.read_volatile() });
    unsafe {
        ptr.write_volatile(0x0);
    }
}
